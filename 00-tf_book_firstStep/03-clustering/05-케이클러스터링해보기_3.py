...
sess = tf.Session()
sess.run(init_op)
print "assignments: ",assignments, sess.run(assignments)

for i in xrange(num_clusters):
    print "i :", i
    eq = tf.equal(assignments, i)
    where = tf.where(eq)
    reshape = tf.reshape(where, [1, -1])
    gather = tf.gather(vectors,reshape)
    reduce_mean = tf.reduce_mean(gather,reduction_indices=[1])
    print "eq :", eq, sess.run(eq)
    print " where :", where, sess.run(where)
    print "reshape :", reshape, sess.run(reshape)
    print "gather :", gather, sess.run(gather)
    print "reduce_mean ", reduce_mean, sess.run(reduce_mean)
    print "--------------------------"
